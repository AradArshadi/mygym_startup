...
def random_past_datetime(days_back=DAYS_BACK):
    ...
    # Realistic gym traffic: morning + evening peaks
    hour = random.choices(
...
def create_booking(gym, customer, plan, status=None, dt=None):
    ...
    for offset in range(5):
        ...
        if created:
            # auto_now_add cannot be passed directly, so update afterward
...
def create_checkins(gym):
    ...
    fields = get_field_names(GymCheckIn)
    created_count = 0

    # Support different project versions:
    # Some versions may call the field "type", others "checkin_type" or "check_in_type".
    ...
    for _ in range(CHECKINS_PER_GYM):
        ...
        if "checked_in_at" in fields:
            ...
            # Usually auto_now_add, but only use this if there is no better check-in timestamp.
...
def create_workouts(customers):
    ...
    fields = get_field_names(WorkoutLog)
    created_count = 0
...
print("GymCheckIn fields:", ", ".join(sorted(get_field_names(GymCheckIn))))
